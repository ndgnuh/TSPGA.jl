...
\begin{frame}
	...
	\begin{itemize}
			| Trọng số đỉnh:
			| Trao đổi chéo, với input: trạng thái $p_1, p_2$, đồ thị $\mathbf{A}$
			\begin{enumerate}
					| $W$ là trọng số các cạnh trong $p_1$
					| $m_1, m_2 = \operatorname{rand}(1\colon (n\div4))$
					| Chọn $m_1, m_2$ cạnh từ $E_1, E_2$ theo trọng số đồ thị $\mathbf{A}$ tăng dần
					| $V_1, V_2$ là tập đỉnh trong các cạnh được chọn
					| Ghép $V_1$ với $V_1 \setminus p_2$ thu được trạng thái con $c_1$
					| Tương tự với $V_2$ và $p_1$ thu được trạng thái con $c_2$
			\end{enumerate}
		| Đột biến:
			\begin{equation*}
			\begin{tikzpicture}
...
\begin{frame}
	...
	\begin{itemize}
			...
			\begin{table}
				\begin{tabular}{lrr}
					\toprule
					\textbf{Thao tác} & \textbf{Thời gian} & \textbf{Không gian}
					\\\midrule\midrule
					Trao đổi chéo & $O(n)$ & $O(n)$
					\\\midrule
					Đột biến & $O(1)$ & $O(n)$
					\\\midrule
					Lựa chọn & $O(m)$ & $O(m)$
...
\begin{frame}
	...
	\begin{figure}
		...
		\end{subfigure}
		\caption{Ước lượng độ phức tạp, kích thước bài toán $n \in \{50, 100, \ldots, 1000\}$.}
	\end{figure}
\end{frame}
...
\begin{frame}{\ft}
	...
	\begin{itemize}
		| Ước lượng trực cảm chi phí trong trao đổi chéo
